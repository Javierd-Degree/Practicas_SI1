...
WITH current_prices AS (SELECT products.price/(1.02^(DATE_PART('year', NOW()) - DATE_PART('year', orderdate)))*quantity AS price, orderdetail.orderid AS orderid, orderdetail.prod_id AS prod_id FROM 
	orders NATURAL JOIN (orderdetail INNER JOIN products ON orderdetail.prod_id=products.prod_id))


UPDATE orderdetail o SET price=ROUND(c.price::numeric,2) FROM current_prices c WHERE o.prod_id=c.prod_id AND o.orderid=c.orderid;
